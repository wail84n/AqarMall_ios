...
open class CropViewController: UIViewController {
    ...
    open override func viewDidLoad() {
        ...
        if self.toolbarItems == nil {
            ...
            let constrainButton = UIBarButtonItem(title: "قلب الصورة", style: .plain, target: self, action: #selector(CropViewController.constrain(_:)))
...
//        actionSheet.addAction(square)
        ...
        let threeByTwo = UIAlertAction(title: "قلب الصورة للأعلى", style: .default) { [unowned self] action in
           // self.cropView?.cropAspectRatio = 2.0 / 3.0
            
            self.cropView?.image = self.cropView?.image?.imageUpMirror()
            
        }
        actionSheet.addAction(threeByTwo)
        
        let threeByFive = UIAlertAction(title: "قلب الصورة للأسفل", style: .default) { [unowned self] action in
            self.cropView?.image = self.cropView?.image?.imageDownMirror()
        }
        actionSheet.addAction(threeByFive)
        
        let fourByThree = UIAlertAction(title: "قلب الصورة لليمين", style: .default) { [unowned self] action in
            self.cropView?.image = self.cropView?.image?.imageRightMirror()
        }
        actionSheet.addAction(fourByThree)
        
        let fourBySix = UIAlertAction(title: "قلب الصورة لليسار", style: .default) { [unowned self] action in
            self.cropView?.image = self.cropView?.image?.imageLeftMirror()
        }
...
//        actionSheet.addAction(widescreen)
        let cancel = UIAlertAction(title: "إلغاء", style: .default) { [unowned self] action in
