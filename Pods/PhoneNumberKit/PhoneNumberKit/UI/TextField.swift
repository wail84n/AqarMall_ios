...
open class PhoneNumberTextField: UITextField, UITextFieldDelegate {
    ...
    open func updatePlaceholder() {
        ...
        if #available(iOS 13.0, *), self.withPrefix {
            ...
            ph.addAttribute(.foregroundColor, value: UIColor.red, range: NSRange(..<firstSpaceIndex, in: example))
            ph.addAttribute(.foregroundColor, value: UIColor.green, range: NSRange(firstSpaceIndex..., in: example))
        }
